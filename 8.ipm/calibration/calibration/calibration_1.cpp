...
CV_IMPL void
Undistort1( const CvArr* srcarr, CvArr* dstarr, const CvMat* Aarr, const CvMat* dist_coeffs, const CvMat* rot_mat, const CvMat* trans_vect );
...
int run_calibration( CvSeq* image_points_seq, CvSize img_size, CvSize board_size,
                     ...
                     CvMat* camera_matrix, CvMat* dist_coeffs, CvMat* rot_vects, CvMat* trans_vects, CvMat** extr_params,
...
void save_camera_params( const char* out_filename, int image_count, CvSize img_size,
						 ...
						 const CvMat* rot_vects, CvMat* trans_vects,
...
/*
	double r[9];
	CvMat rot_vect, rot_mat = cvMat( 3, 3, extr_params->type, r );
	cvGetRow( rot_vects, &rot_vect, 0 );
	cvRodrigues2( &rot_vect, &rot_mat );
	cvWrite( fs, "rot_mat0", &rot_mat );
*/
...
{
    IplImage* undistort = 0;
	//const char* undistort_imagename = "undistort_iamge";
...
calibrate:
            ...
            int code = run_calibration( image_points_seq, img_size, board_size,
                square_size, aspect_ratio, flags, &camera, &dist_coeffs, &rot_vects, &trans_vects, &extr_params,
            ...
            save_camera_params( out_filename, image_count, img_size,
                ...
                &camera, &dist_coeffs, &rot_vects, &trans_vects, write_extrinsics ? extr_params : 0,
    ...
    /*undistort using camera_matrix and distortion_coefficients: */
	f = fopen( input_filename, "rt" );
	cvNamedWindow( "undistort", 1 );
	char outname[50] = "E:\\calibration\\undistort\\1.jpg";
	int m = strlen( outname );
	for( int i = 0;; i++ )
    {
        IplImage *view = 0;
		IplImage* view_gray1;
		if( f && fgets( imagename, sizeof(imagename)-2, f ))
        {
            ...
            if( !view )
				break;
			view_gray1 = cvCreateImage(cvSize(view->width, view->height), IPL_DEPTH_8U, 3);
           			
			double r[9];
			CvMat trans_vect, rot_vect, rot_mat = cvMat( 3, 3, extr_params->type, r );
			cvGetRow( &rot_vects, &rot_vect, i );
			cvGetRow( &trans_vects, &trans_vect, i);
			cvRodrigues2( &rot_vect, &rot_mat );
			Undistort1( view, view_gray1, &camera, &dist_coeffs, &rot_mat, &trans_vect );
			cvShowImage( "undistort", view_gray1 );
			cvWaitKey(500);
			cvSaveImage( outname, view_gray1 );
			outname[m-5]++;
        }
		cvReleaseImage( &view );
	}
