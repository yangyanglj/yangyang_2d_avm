...
int main(int argc, char** argv) 
{
	...
	Mat replaceImg_top = imread("yy_top.JPG");
...
//top
	...
	src_corners[0] = Point(586, 346);

	src_corners[1] = Point(715, 351);

	src_corners[2] = Point(749, 409);

	src_corners[3] = Point(544, 406);
	// 目标平面四个角坐标

	dst_corners[0] = Point(104,330);

	dst_corners[1] = Point(104, 321);

	dst_corners[2] = Point(242, 321);

	dst_corners[3] = Point(242, 330);
#endif
//top
#if 0
	// 定义两个平面上四个角坐标
...
//black
	...
	src_black_corners[0] = Point(src.cols/3, src.rows/3*2);

	src_black_corners[1] = Point(src.cols/3*2, src.rows/3*2);

	src_black_corners[2] = Point(src.cols, src.rows);

	src_black_corners[3] = Point(0, src.rows);
	// 目标平面四个角坐标
...
//black
	...
	warpPerspective(black_rotate, output_img_black, h_black, black_rotate.size());
	// create mask
	Mat m1_black = Mat::zeros(black_rotate.size(), CV_8UC1);
	m1_black = Scalar(255);
	Mat mask_output_black;
	warpPerspective(m1_black, mask_output_black, h_black, black_rotate.size());
	// use mask
	Mat result1_black;
	add(output_img_black, output_img_black, result1_black, mask_output_black);
	Mat result2_black;
	bitwise_not(mask_output_black, mask_output_black);
	add(black_rotate, result1_black, result2_black, mask_output_black);
	Mat result_black;
	add(result1_black, result2_black, result_black);
...
//right
	...
	imshow("right Result", result_left);
#endif
	waitKey(0);
}
