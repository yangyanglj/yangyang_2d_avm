...
int main()
{
    ...
    *************************************************************************/
    ...
    for (int i = 0 ; i != image_count ; i++)
    {
	//fisheye::initUndistortRectifyMap(intrinsic_matrix,distortion_coeffs,R,getOptimalNewCameraMatrix(intrinsic_matrix, distortion_coeffs, image_size, 1, image_size, 0),image_size,CV_32FC1,mapx,mapy);
    ...
    *************************************************************************/
#if 0
	Mat distort_img = imread("yy_top.JPG", 1);
	
	Mat undistort_img;
	Mat intrinsic_mat(intrinsic_matrix), new_intrinsic_mat;
	intrinsic_mat.copyTo(new_intrinsic_mat);
        //调节视场大小,乘的系数越小视场越大
        new_intrinsic_mat.at<double>(0, 0) *= 0.6;//0.8
        new_intrinsic_mat.at<double>(1, 1) *= 0.6;
        //调节校正图中心，建议置于校正图中心
...
#endif
#if 1
        Mat testImage = imread("black.JPG", 1);
//相机经过畸变校正后设置的新的内参：new_intrinsic_mat	
	Mat intrinsic_mat(intrinsic_matrix), new_intrinsic_mat;
	intrinsic_mat.copyTo(new_intrinsic_mat);
//调节新的内参系数，调节视场大小，乘的系数越小视场越大
	 new_intrinsic_mat.at<double>(0, 0) *= 0.6;//0.8
        new_intrinsic_mat.at<double>(1, 1) *= 0.6;
        //调节校正图中心，建议置于校正图中心
        new_intrinsic_mat.at<double>(0, 2) = 0.5 * testImage.cols;
        new_intrinsic_mat.at<double>(1, 2) = 0.5 * testImage.rows;
	...
	//initUndistortRectifyMap(intrinsic_matrix,distortion_coeffs,R,intrinsic_matrix,image_size,CV_32FC1,mapx,mapy);
        Mat tt = testImage.clone();
	remap(testImage,tt,mapx, mapy, INTER_LINEAR);
	//cout<<"cameraMatrix:"<<cameraMatrix<<endl;
	//cout<<"distCoeffs:"<<distCoeffs<<endl;
	imshow("remap", tt);
        imwrite("TestOutput.jpg",tt);
	waitKey(0);
//H矩阵的初始化
#endif
#if 0
